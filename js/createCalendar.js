...
function getCalendarEvents(startDate, endDate) {
	...
	const username = Calendar.user.getUsername();
    ...
    docClient.query(params, function(err, data) {
        if (err) {
			...
			for(i = 0; i < items.length; i++) {
				...
				// store the event data for access later
				const eventId = items[i].eventId;
				allEvents[eventId] = items[i];
				...
				if(eventDate >= startDate && eventDate <= endDate) {
					...
					console.log("table day", tableDay);
					if((hours >= 7 && hours <= 20) && (minutes == 0 || minutes == 30)) {
						...
						// set the value of the cell to the eventId, for mapping
						document.getElementById(tableCell).value = eventId;

						console.log("each cell", tableCell);
						console.log("items", items[i].eventInfo.category);
						
						// add click listener for event view
						$("#" + tableCell).click(function() {
							const eventId = this.value;
							const eventDetails = allEvents[eventId];

							Cookies.set('eventInfo', eventDetails.eventInfo);
		
							// transition to event view
